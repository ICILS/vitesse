...
<template>
  ...
  >
    ...
    <!-- ---------------------------------------------- -->
    <div class="pa-4">
      <LogoDark v-if="!customizer.darktheme && customizer.navbarColor === '#f6f6f6'" />
      <LogoLight v-else />
    </div>
    <v-app-bar-nav-icon
      color="inherit" class="hidden-sm-and-down"
    ...
    <v-btn text icon color="inherit" @click="searchbox">
      <!-- <vue-feather type="search" class="feather-sm" /> -->
      <div i-feather-search class="feather-sm" />
    </v-btn>
    <v-sheet v-if="showSearch" class="searchinput pa-3" elevation="10">
      <v-text-field
        color="primary" label="Search" placeholder="Search Now" variant="outlined" append-icon="i-mdi-close"
        density="compact" hide-details @click:append="searchbox"
      />
    </v-sheet>
    <!--- /Search part -->
      ...
      <template #activator="{ props }">
        <v-btn icon color="inherit" v-bind="props">
          <v-badge color="error" dot>
            <!-- <v-icon>i-mdi-message</v-icon> -->
            <div i-mdi-message text-2xl />
          </v-badge>
        </v-btn>
      </template>
    ...
    <v-menu anchor="bottom end" origin="auto">
      <template #activator="{ props }">
        <v-btn icon color="inherit" v-bind="props">
          <v-badge color="error" dot>
            <!-- <v-icon>i-mdi-bell</v-icon> -->
            <div i-mdi-bell text-2xl />
          </v-badge>
        </v-btn>
      </template>
      ...
      <v-list class="pa-6" elevation="10" rounded="lg" max-width="300">
        ...
        <v-list-item
          v-for="(item, i) in notifications" :key="i" class="pa-3 mt-2" :value="item" rounded="lg"
          :title="item.title" :subtitle="item.desc" @click="href"
        >
          <template #prepend>
    ...
    <v-menu anchor="bottom end" origin="auto" min-width="300">
      ...
      <v-list class="pa-6" elevation="10" rounded="lg">
        <h4 class="font-600 fs-18">
          User Profile
        </h4>
        <div class="d-flex align-center my-4">
          <img src="@/assets/images/users/user2.jpg" alt="Julia" class="rounded-circle" width="90">
          <div class="ml-4">
            <h4 class="font-600 text-lg">
              Julia Roberts
            </h4>
            <!-- TODO:  -->
            <span class="text-subtitle-1 font-light">Administrator</span>
            <div class="d-flex align-center">
              <!-- <vue-feather type="mail" size="16" class="d-flex grey--text" /> -->
              <div i-feather-mail class="d-flex grey--text h-16px" />
              <span class="text-subtitle-1 font-light ml-1">info@flexy.com</span>
            </div>
          </div>
        </div>
        <v-list-item
          v-for="(item, i) in userprofile" :key="i" class="pa-3 mb-2" :value="item" :title="item.title"
