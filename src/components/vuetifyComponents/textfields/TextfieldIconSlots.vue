    ...
    <div class="mt-4">
      <v-form>
        <v-container>
          <v-row>
            <v-col cols="12">
              <v-text-field v-model="message" clearable label="Message" type="text" variant="outlined">
                <template #prepend>
                  <v-tooltip location="bottom">
                    <template #activator="{ props }">
                      <v-icon v-bind="props" icon="i-mdi-help-circle-outline" />
                    </template>
                    ...
                    <v-card>
                      <v-card-text class="pa-6">
                        <v-btn color="primary" size="large" variant="text" @click="clickMe">
                          <v-icon icon="i-mdi-target" start />
